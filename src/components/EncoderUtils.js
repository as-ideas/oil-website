...
EncoderUtils.encodeBinaryBitStringToBase64 = (binaryValue) => {
...
EncoderUtils.encodeField = ({decodedObject, field}) => {
  const {name, type, numBits, encoder, validator} = field;
  if (typeof validator === 'function') {
    if (!validator(decodedObject)) {
      return '';
    }
  }
  if (typeof encoder === 'function') {
    return encoder(decodedObject);
  }

  const bitCount = typeof numBits === 'function' ? numBits(decodedObject) : numBits;

  const inputValue = decodedObject[name];
...
EncoderUtils.decodeField = ({input, output, startPosition, field}) => {
